...
def projected_gradient_descent_hinge(
    model_fn,
    x,
    eps,
    eps_iter,
    ...
    rand_minmax=None,
    sanity_checks=True,
    early_stop=False,                    # added by me
    loss_fn=torch.nn.CrossEntropyLoss(), # added by me
):
    """
    ...
    if y is None:
        ...
        y = model_fn(x) # for hinge loss for attacking single ECOC members (binany models)
        y = torch.tanh(y) 
    ...
    while i < nb_iter:
        adv_x = fast_gradient_method(model_fn, adv_x, eps_iter, norm, clip_min=clip_min, clip_max=clip_max, y=y, targeted=targeted, loss_fn=loss_fn,)
